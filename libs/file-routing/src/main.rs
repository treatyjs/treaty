...
use treaty_file_routing::{
    generate_routing, AngularRoute, DynamicSegmentStyle, FederationRemote, FileRoutingConfig,
    GeneratedRouting, PartialFileRoutingConfig, RealFsDirTree,
};
...
fn run(args: &[String]) -> Result<Option<String>, String> {
    ...
    let rendered = match cli.emit {
        ...
        EmitFormat::Ts => render_ts(&routing, &cli.import_base),
    };
...
// ---------------------------------------------------------------------------
// TypeScript route-module emission.
// ---------------------------------------------------------------------------
...
/// `import(...)` loaders (default `../../`, i.e. resolving `routes/foo.treaty`
/// from a `src/generated/` module). The output is byte-for-byte deterministic
/// for a given routing + base.
fn render_ts(routing: &GeneratedRouting, import_base: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn join_import_handles_base_variants() {
        assert_eq!(join_import("../../", "routes/x.treaty"), "../../routes/x.treaty");
        assert_eq!(join_import("../..", "routes/x.treaty"), "../../routes/x.treaty");
        assert_eq!(join_import("", "routes/x.treaty"), "routes/x.treaty");
        assert_eq!(join_import("@app", "routes/x.treaty"), "@app/routes/x.treaty");
    }
    ...
    fn ts_emit_matches_known_routes_table() {
        ...
        for (path, entry) in [
            ...
            assert!(
                ts.contains(&format!("import({})", json_string(entry))),
