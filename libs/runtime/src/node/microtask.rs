//! queueMicrotask — enqueues a callback onto the shared event-loop microtask queue. Surfaced as an eager global; this module backs it.
//!
//! Lazy: built only on first `require`/`import` of `node:microtask` (unless surfaced as a global by
