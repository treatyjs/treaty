//! `node:console` — the Console class backing the global console (log/info/warn/error/debug, format).
//!
//! Lazy: built only on first `require`/`import` of `node:console` (unless surfaced as a global by
...
/// Uniform per-module entry. Returns the `node:console` exports object.
