 ...
 * @module
 *
 * Library build for `@treaty/rolldown`.
 *
 ...
 *
 * `@treaty/rolldown` declares `"type": "module"`, so `dist/index.js` is ESM. Its
 * runtime dependencies — `@treaty/compiler` and especially `@treaty/ts-vite` (the
 * shared Angular partial-declaration linker) — MUST stay EXTERNAL, i.e. real
 * `import ... from "@treaty/ts-vite"` statements in the published bundle, NOT
 * inlined copies.
 *
 * Why this matters: `@treaty/ts-vite` is CommonJS and loads the native Rust linker
 * addon with `require("@treaty/authoring-node")`. If esbuild INLINES ts-vite into
 * this ESM bundle, that bare `require` is rewritten into esbuild's "Dynamic
 * require of \"@treaty/authoring-node\" is not supported" THROWING shim. A
 * published `@treaty/rolldown` built that way ships with the partial-Angular
 * linker silently disabled, so it emits un-linked `ɵɵngDeclare*` that crashes at
 * runtime ("needs the JIT compiler / @angular/compiler is not available").
 *
...
const options = {
	...
	sourcemap: true,
	minify,
	// Keep node_modules-style bare deps external (rolldown, esbuild, node builtins,
	// ...). The explicit list + plugin below cover the @treaty/* deps that resolve
	// under libs/ and would otherwise be inlined despite this flag.
	packages: 'external',
...
function emitDeclarations() {
	...
	if (!existsSync(bin)) {
		throw new Error(
			`[treaty-rolldown] tsgo not found at ${bin}; cannot emit declarations (tsc is forbidden).`,
		)
	}
...
if (watch) {
	...
	await ctx.watch()
	console.log('[treaty-rolldown] watching for changes...')
} else {
	// Start from a clean dist so an earlier build leaves no orphaned per-module
	// `.js` (server-chunks is bundled INTO index.js; the package `exports` expose
	// only `.`). Declarations are re-emitted below.
	rmSync(resolve(here, 'dist'), { recursive: true, force: true })
	await esbuild.build(options)
	emitDeclarations()
	console.log('[treaty-rolldown] build complete (dist/index.js ESM + declarations).')
}
