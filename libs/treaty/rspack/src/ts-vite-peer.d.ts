 ...
 * `@treaty/rspack` consumes.
 *
 * `@treaty/rspack` reuses the Angular partial-declaration linker from `@treaty/ts-vite` (one source
 * of truth, Rust-backed via `@treaty/authoring-node`.`linkPartial`) rather than re-implementing it.
 * `@treaty/ts-vite` is a CommonJS package whose published types are not part of `@treaty/rspack`'s
 ...
 * `import('vite').Plugin[]`, which is unusable by Rspack; rspack wires the linker through its own
 * loader instead.
 */
