 ...
 *   2. Find the FULL expected golden — the per-case `.js` containing
 *      `ɵɵdefineComponent` (NOT GOLDEN_PARTIAL.js / ngDeclareComponent goldens,
 *      which are skipped as partial-only).
 *   3. compile_component_source(src). errors => SKIPPED (categorised by reason).
 *   4. FRAGMENT-MATCH our emitted ɵɵdefineComponent block against the golden's
 *      defineComponent block: split the expected on the `…`/`// ...` ellipsis,
...
// ---------------------------------------------------------------------------
// Golden selection + defineComponent block extraction.
// ---------------------------------------------------------------------------

/** Extract the FIRST balanced `ɵɵdefineComponent({ ... })` argument block (incl. the
 *  surrounding parens) from a chunk of emitted JS. Returns the `({...})` slice or null. */
...
/**
 * Pick the FULL golden for a case: the referenced expected `.js` whose content
 * contains `ɵɵdefineComponent`. Skips GOLDEN_PARTIAL.js / ngDeclareComponent-only
 * goldens. Returns { block, file } or null.
 */
function pickFullGolden(caseDir, goldens) {
  for (const g of goldens) {
    ...
    if (!content.includes('ɵɵdefineComponent')) continue;
    const block = extractDefineComponentBlock(content);
    if (block) return { block, file: g };
  }
  return null;
}
...
function main() {
  ...
  for (const tc of cases) {
    ...
    // Find the FULL golden (one containing ɵɵdefineComponent). Skip cases whose only
    // golden is partial / ngDeclare.
    ...
    // Compile the SOURCE via the Rust front-end.
    const src = fs.readFileSync(inputPath, 'utf8');
    let rustOut;
    try {
      rustOut = rust.compileSource(src);
    } catch (err) {
    ...
    if (!rustOut.code || !rustOut.code.includes('ɵɵdefineComponent')) {
      bumpSkip(result, 'no-define-emitted', id);
      continue;
    }

    const rustBlock = extractDefineComponentBlock(rustOut.code);
    if (!rustBlock) {
      bumpSkip(result, 'no-define-block', id);
      continue;
    }
