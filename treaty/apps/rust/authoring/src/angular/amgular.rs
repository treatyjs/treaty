...
impl<'a> VisitMut<'a> for Transformer<'a> {
    ...
    fn visit_program(&mut self, program: &mut Program<'a>) {
        ...
        self.typescript
            .as_mut()
            .map(|t| t.transform_program(program));
        self.visit_statements(&mut program.body);

        self.react_jsx
            .as_mut()
            .map(|t| t.add_react_jsx_runtime_imports(program));
        self.decorators
            .as_mut()
            .map(|t| t.transform_program(program));
    ...
    fn visit_assignment_expression(&mut self, expr: &mut AssignmentExpression<'a>) {
        ...
        self.es2015_function_name
            .as_mut()
            .map(|t| t.transform_assignment_expression(expr));
    ...
    fn visit_statements(&mut self, stmts: &mut oxc_allocator::Vec<'a, Statement<'a>>) {
        self.typescript
            .as_mut()
            .map(|t| t.transform_statements(stmts));

        for stmt in stmts.iter_mut() {
            self.visit_statement(stmt);
        }
        // TODO: we need scope id to insert the vars into the correct statements
        self.es2021_logical_assignment_operators
            .as_mut()
            .map(|t| t.add_vars_to_statements(stmts));
        self.es2020_nullish_coalescing_operators
            .as_mut()
            .map(|t| t.add_vars_to_statements(stmts));
        self.es2016_exponentiation_operator
            .as_mut()
            .map(|t| t.add_vars_to_statements(stmts));
        self.es2015_arrow_functions
            .as_mut()
            .map(|t| t.transform_statements(stmts));
    }
    ...
    fn visit_statement(&mut self, stmt: &mut Statement<'a>) {
        self.typescript
            .as_mut()
            .map(|t| t.transform_statement(stmt));
        self.decorators
            .as_mut()
            .map(|t| t.transform_statement(stmt));
    ...
    fn visit_declaration(&mut self, decl: &mut Declaration<'a>) {
        ...
        self.typescript
            .as_mut()
            .map(|t| t.transform_declaration(decl));
        self.decorators
            .as_mut()
            .map(|t| t.transform_declaration(decl));
    }
    ...
    fn visit_expression(&mut self, expr: &mut Expression<'a>) {
        ...
        self.react_jsx
            .as_mut()
            .map(|t| t.transform_expression(expr));
        self.regexp_flags
            .as_mut()
            .map(|t| t.transform_expression(expr));
        ...
        self.es2015_arrow_functions
            .as_mut()
            .map(|t| t.transform_expression(expr));
        self.es2015_instanceof
            .as_mut()
            .map(|t| t.transform_expression(expr));
        self.es2016_exponentiation_operator
            .as_mut()
            .map(|t| t.transform_expression(expr));
        self.es2015_template_literals
            .as_mut()
            .map(|t| t.transform_expression(expr));
        self.es2015_new_target
            .as_mut()
            .map(|t| t.transform_expression(expr));
    ...
    fn visit_catch_clause(&mut self, clause: &mut CatchClause<'a>) {
        ...
        self.es2019_optional_catch_binding
            .as_mut()
            .map(|t| t.transform_catch_clause(clause));
    ...
    fn visit_object_expression(&mut self, expr: &mut ObjectExpression<'a>) {
        ...
        self.es2015_function_name
            .as_mut()
            .map(|t| t.transform_object_expression(expr));
        self.es2015_duplicate_keys
            .as_mut()
            .map(|t| t.transform_object_expression(expr));
    ...
    fn visit_object_property(&mut self, prop: &mut ObjectProperty<'a>) {
        ...
        self.es2015_shorthand_properties
            .as_mut()
            .map(|t| t.transform_object_property(prop));
        self.es3_property_literal
            .as_mut()
            .map(|t| t.transform_object_property(prop));
    ...
    fn visit_class_body(&mut self, class_body: &mut ClassBody<'a>) {
        self.es2022_class_static_block
            .as_mut()
            .map(|t| t.transform_class_body(class_body));
    ...
    fn visit_variable_declarator(&mut self, declarator: &mut VariableDeclarator<'a>) {
        ...
        self.es2015_function_name
            .as_mut()
            .map(|t| t.transform_variable_declarator(declarator));
    ...
    fn visit_method_definition(&mut self, def: &mut MethodDefinition<'a>) {
        ...
        self.typescript
            .as_mut()
            .map(|t| t.transform_method_definition(def));
