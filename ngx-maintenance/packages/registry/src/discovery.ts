...
export function discoverStale(
  ...
  for (const lib of libs) {
    ...
    const majorsBehind = latestAngular - lib.currentAngular;
    const inactiveForMs = now - lib.lastCommitMs;
    findings.push({
      metadata: lib,
      majorsBehind,
      inactiveForMs,
      reason: staleReason(lib, latestAngular, majorsBehind, inactiveForMs),
    });
  }
  return findings;
}
