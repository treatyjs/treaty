...
| --- | --- | --- |
| `src/components/todo-list.treaty` | `.treaty` single-file component | A top **macro fence** (```` ``` ````) that computes **render-time data** at compile time (zero runtime cost); a **TypeScript-by-default** body (no `<script>` wrapper); an HTML `<template>`; and a `<style lang="scss">` block with nesting + SCSS variables. Calls the **API**-transport server fns. |
...
### Server functions (three transports)

The function **bodies never enter the Ivy/client bundle**; only the typed call
site in a component does (it compiles to the resource/Eden binding).
...
| File | Shows |
| --- | --- |
| `src/routes/app.routes.ts` | One eager route plus two **lazy** feature routes (`loadComponent` and `loadChildren`). Treaty's `deriveExposesFromRoutes` turns every lazy boundary into an independently deployable **Module Federation remote** with no hand-written `exposes` map. |
...
The two lazy routes (`dashboard`, `profile`) become independently deployable
remotes; the eager index route stays in the host.
