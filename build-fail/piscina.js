...
var _DirectlyTransferable_value,
  _ArrayBufferViewTransferable_view,
  _Piscina_pool;
...
class DirectlyTransferable {
  ...
  get [((_DirectlyTransferable_value = new WeakMap()),
  common_1.kTransferable)]() {
...
class ArrayBufferViewTransferable {
  ...
  get [((_ArrayBufferViewTransferable_view = new WeakMap()),
  common_1.kTransferable)]() {
...
class TaskInfo extends async_hooks_1.AsyncResource {
  ...
  get [common_1.kQueueOptions]() {
    return common_1.kQueueOptions in this.task
      ? this.task[common_1.kQueueOptions]
      : null;
  }
}
...
class WorkerInfo extends AsynchronouslyCreatedResource {
  constructor(worker, port, onMessage) {
    ...
    this.sharedBuffer = new Int32Array(
      new SharedArrayBuffer(common_1.kFieldCount * Int32Array.BYTES_PER_ELEMENT)
    );
  }
  destroy() {
  ...
  processPendingMessages() {
    ...
    const actualResponseCount = Atomics.load(
      this.sharedBuffer,
      common_1.kResponseCountField
    );
    if (actualResponseCount !== this.lastSeenResponseCount) {
      this.lastSeenResponseCount = actualResponseCount;
      let entry;
      while (
        (entry = (0, worker_threads_1.receiveMessageOnPort)(this.port)) !==
        undefined
      ) {
...
class ThreadPool {
  constructor(publicInterface, options) {
    ...
    this.runTime = 0; //(0, perf_hooks_1.createHistogram)();
    this.waitTime = 0; //(0, perf_hooks_1.createHistogram)();
    ...
    if (options.maxQueue === 'auto') {
      ...
      this.options.maxQueue =
        (_a = options.maxQueue) !== null && _a !== void 0
          ? _a
          : kDefaultOptions.maxQueue;
    }
  ...
  _addNewWorker() {
    ...
    const worker = new worker_threads_1.Worker(
      (0, path_1.resolve)(__dirname, 'worker.js'),
      {
    ...
    function onMessage(message) {
      ...
      if (taskInfo === undefined) {
        const err = new Error(
          `Unexpected message from Worker: ${(0, util_1.inspect)(message)}`
        );
    ...
    worker.on('message', (message) => {
      message instanceof Object && common_1.READY in message
        ? onReady()
  ...
  _onWorkerAvailable(workerInfo) {
    while (
      ...
      // this.waitTime.record(0) //toHistogramIntegerNano(now - taskInfo.created));
...
class Piscina extends events_1.EventEmitterAsyncResource {
  constructor(options = {}) {
    ...
    if (
      ...
      !(0, common_1.isTaskQueue)(options.taskQueue)
    ) {
  ...
  runTask(task, transferList, filename, signal) {
    ...
    if (
      (typeof transferList === 'object' && !Array.isArray(transferList)) ||
  ...
  get threads() {
    const ret = [];
    for (const workerInfo of __classPrivateFieldGet(this, _Piscina_pool, 'f')
      .workers) {
  ...
  get waitTime() {
    return createHistogramSummary({ mean: 0, stddev: 0, min: 0, max: 0 });
  }

  get runTime() {
    return createHistogramSummary({ mean: 0, stddev: 0, min: 0, max: 0 });
    return createHistogramSummary(
      __classPrivateFieldGet(this, _Piscina_pool, 'f').runTime
    );
  }
  get utilization() {
    // count is available as of Node.js v16.14.0 but not present in the types
    const count = __classPrivateFieldGet(this, _Piscina_pool, 'f').runTime
      .count;
    if (count === 0) {
      return 0;
    }
    ...
    const capacity =
      this.duration *
      __classPrivateFieldGet(this, _Piscina_pool, 'f').options.maxThreads;
    const totalMeanRuntime =
      (__classPrivateFieldGet(this, _Piscina_pool, 'f').runTime.mean / 1000) *
      count;
  ...
  get duration() {
    return (
      perf_hooks_1.performance.now() -
      __classPrivateFieldGet(this, _Piscina_pool, 'f').start
    );
  }
  get needsDrain() {
